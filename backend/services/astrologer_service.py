...
# Import settings for personas file path
try:
    from backend.config.settings import ASTROLOGER_PERSONAS_FILE
except ImportError:
    # Fallback if importing as standalone
    ASTROLOGER_PERSONAS_FILE = Path(__file__).resolve().parent.parent.parent / "data" / "astrologer_personas.json"
...
class AstrologerManager:
    ...
    def __init__(self, personas_file: str = None):
        ...
        """
        self.personas_file = str(personas_file or ASTROLOGER_PERSONAS_FILE)
